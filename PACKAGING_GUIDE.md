...
PyInstaller 默认会生成两个目录：

- `build/`：中间构建目录，存放临时文件、分析缓存和打包过程中的产物。一般不需要交付给用户，可以删除。
- `dist/`：最终输出目录，真正可运行的 EXE 会放在这里。你要分发给用户的，就是 `dist/` 里的成品。

如果你看到这两个目录，这是正常现象，不是打包失败。
...
双击 `微信教育代理.exe` 后，会默认进入 GUI 模式，不需要再手动传 `gui` 参数。
...
4. ✅ 双击运行测试，确认会直接打开 GUI
5. ✅ 分发整个 `dist/微信教育代理/` 目录，不需要 `build/`
